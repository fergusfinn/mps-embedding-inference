...
# dmon sweep: measure FLOP utilization, SM%, mem bandwidth%, power at different concurrency levels
...
RESULTS_DIR="$DIR/results/dmon_${MODEL_SHORT}_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$RESULTS_DIR"
...
for C in $CONCURRENCY_LEVELS; do
    echo "=========================================="
    echo "Testing C=$C (max-num-seqs=$C)"
    echo "=========================================="

    # Start vLLM with enforce-eager (matching nsys runs)
    ...
    python3 "$DIR/gen_benchmark.py" --base-url "http://localhost:$VLLM_PORT" \
        --duration 10 --concurrency "$C" --max-tokens 64 --model "$MODEL" > /dev/null 2>&1
